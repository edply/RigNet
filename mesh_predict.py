...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_filepath', required=True, type=str, help='filepath of .obj file to be used for rig prediction') 
    parser.add_argument('--bandwidth', default=0.0429, type=float, help='bandwith value to use for inference') 
    parser.add_argument('--threshold', default=1e-5, type=float, help='threshold value to use for inference') 
    parser.add_argument('--transfer_to_original', action='store_true', help='set flag to transfer the rig prediction to the original mesh') 
    parser.add_argument('--maya_installation_dir', type=str, help='set to Maya installation path to output .fbx file containing mesh and rig') 


    args = parser.parse_args()

    input_folder = os.path.dirname(args.input_filepath)
    ...
    mesh_ori = o3d.io.read_triangle_mesh(args.input_filepath)
    mesh_remesh = mesh_ori.simplify_quadric_decimation(4000)  # adjust vertices between 1K - 5K

    mesh_remesh_filepath = os.path.join(input_folder, f"{os.path.splitext(input_filename)[0]}_remesh{os.path.splitext(input_filename)[1]}")
    print(f"Writing mesh with reduced vertices to {mesh_remesh_filepath}.")
    o3d.io.write_triangle_mesh(mesh_remesh_filepath, mesh_remesh)


    # create data used for inferece
    print(f"creating data for mesh {input_filename}")

    data, vox, surface_geodesic, translation_normalize, scale_normalize = create_single_data(mesh_remesh_filepath)
    data.to(device)

    print("predicting joints")
    data = predict_joints(data, vox, jointNet, threshold, bandwidth=bandwidth,
                          mesh_filename=mesh_remesh_filepath.replace("_remesh.obj", "_normalized.obj"))
    data.to(device)
    ...
    pred_rig = predict_skinning(data, pred_skeleton, skinNet, surface_geodesic,
                                mesh_remesh_filepath.replace("_remesh.obj", "_normalized.obj"),
    ...
    print("Saving result")
    rig_output_filepath = f"{os.path.splitext(mesh_remesh_filepath)[0]}_rig.txt"
    pred_rig.save(rig_output_filepath)

    if args.transfer_to_original:
        # here we use original mesh tesselation (without remeshing)
        rig_output_filepath = f"{os.path.splitext(args.input_filepath)[0]}_rig.txt"
        pred_rig = tranfer_to_ori_mesh(args.input_filepath, mesh_remesh_filepath, pred_rig)   
        pred_rig.save(rig_output_filepath)

    if args.maya_installation_dir is not None:
        print("Merging rig into .fbx file...")

        if args.transfer_to_original:
            maya_mesh_filepath = args.input_filepath
        else:
            maya_mesh_filepath = mesh_remesh_filepath

        mayapy_path = os.path.join(args.maya_installation_dir, 'bin', 'mayapy')
        subprocess.run([mayapy_path, 'mesh_rig_merge.py', '--input_mesh_filepath', maya_mesh_filepath, '--input_rig_filepath', rig_output_filepath])
